...
LLB and **Highland Park** anchor the `71B` vs **P10** contrast: same FY26 process, very different baseline poverty and transit dependence — and a different **rider-level** outcome (Marcus's bus still runs; Denise's sole-route stop disappears).

| Metric | Lincoln-Lemington–Belmar | Highland Park | Contrast |
|--------|--------------------------|---------------|----------|
| **Population** | 4,485 | 6,400 | LLB smaller |
| **Below poverty** | 32.6% (1,356 residents) | ~4.8% (305 residents) | **~6.8× gap** |
| **Transit-commute proxy** | 25.8% (383 workers) | ~11.1% (~447 workers) | **~2.3× gap** |
| **Routes before FY26** | 8 | 5 | |
| **Routes eliminated** | 2 (P10, P17) | 1 (P10) | |
| **Routes reduced** | 5 (001, 074, 075, 082, 091) | 4 (071A, 071B, 075, 087) | |
| **Routes unaffected** | 1 (079) | 0 | |
| **% routes impacted** | **87.5%** | 100% | |

### Why P10 (a "flyer") is defensible

No 30%+ poverty neighborhood in the FY26 dataset loses an eliminated *local* bus where **sole-route** stops are orphaned the way **E56360** is for Denise in LLB. The eliminated routes hitting 30%+ poverty neighborhoods are commuter flyers — a direct consequence of PRT's efficiency logic, which targets low-ridership routes regardless of the poverty profile of who boards them.
...
|------|------|----------|----------------|----------------------------|
| **A** | Marcus | Choice rider | Highland Park | **71B** — minor reduction; BRT spine (University Line); neighborhood profile lists P10 + other locals reduced, but Marcus's story stays on **71B** |
...
- Profile table: **`data/fy26_route_n_profiles_all.csv`** — Highland Park and Lincoln-Lemington-Belmar rows drive poverty / transit-commute figures cited in copy.
- LLB: **32.6%** below poverty, **25.8%** transit-commute proxy (workers), pop **4,485**. **7 of 8 routes impacted.**
- Highland Park: **~4.8%** below poverty, **~11.1%** transit-commute proxy, pop **6,400**. **P10 eliminated** in that polygon; **071A, 071B, 075, 087 reduced** — Marcus narrative emphasizes **71B** (minor reduction per `FY26_route_status_all.csv`).
- Highland Park median income and no-car share in Marcus's card include **ILLUSTRATIVE** labels — confirm against ACS before presenting as final fact.
...
- **7:10am** — Walks to 71B stop (Highland Park)
...
- Start: **Home**
- **6 min** — first mile: walk to 71B stop (Highland Park) → **71B stop**
...
- **6 min** — first mile: walk → **office**
- *71B still runs with minor trim; drive alternative ~20 min, $14–18/day parking*
...
- **After cuts:** `PersonaDayCard` phase `after` — Marcus: longer waits / drive option, **71B** still runs; Denise: P10 elimination, orphaned stop, 7-min walk to major-reduced Rt 74, compound impact across 7 of 8 routes.
